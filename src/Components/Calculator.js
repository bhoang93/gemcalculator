...
const Calculator = ({
  ...
  const milestone = (days, loggedIn) => {
    ...
    if (extraDays < 60) {
      const pre60milestones = (gem10, gem15, gem20, gem30, gem60) => {
        // Takes how many gems a player will reach on each milestone.
      ...
      if (extraDays >= 30) {
        // If the user has logged in more than 30 days, they can get the bonuses for day 60 but not 20, 15 or 10.
    ...
    while (extraDays.toString().length > 2) {
      // This while loop will remove the first digit until there are only 2 left.
      extraDays = Number(extraDays.toString().substring(1));
    }

    if (extraDays > 50) {
      // This will determine how close they are to the next 50/100 day milestone.
      milestoneGems += 10;
    ...
    if (milestoneGems % 10 === 0) {
      // If statement to determine if the next milestone will give them 5 or 10 gems, then continue from there.
  ...
  const total = (days, isGlobal, daysLoggedIn) => {
    // Find total gems.
    return (
      days +
      fortnight(days) +
      milestone(days, daysLoggedIn) +
      chopperman(days) +
      colosseum(days) +
      storyModeGems +
      Number(currentGems) +
      tmGemsCalc(days, tmGemsInput) +
      newStoryGems(days)
    );
  };
    ...
    <div>
      <Animated animationIn="flipInX" isVisible={isVisible}>
        ...
        <p>{`${chopperman(
          days,
          isGlobal
        )} gems from weekly Chopperman missions.`}</p>
        ...
        {tmGems === true ? (
          <p>{`${tmGemsCalc(
            days,
            tmGemsInput
          )} gems from Treasure Map Mode.`}</p>
        ) : (
          <p />
        )}
        <div className="chopper">
          <img className="chopperimg" src={chopper} alt="Chopper Says" />
          <p className="speech-bubble">{`You will have at least ${total(
            days,
            isGlobal,
            daysLoggedIn
          )} gems in ${days} day(s).`}</p>
        </div>
